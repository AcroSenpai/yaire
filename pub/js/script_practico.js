$(function(){
  ...
  function generar_cal()
  {
    ...
    if(dia!="")
    {
      ...
      profe = $("#profesor").val();
      zona = $("#zona").val();
      $.post("/practico/crear_tabla", {num1:numeros[0],profe:profe,zona:zona,mes:mes,año:año}, function(data){
        $("#mapa").append(data);
  ...
  /*Cuando cambias de profesor vuelve a cargar la tabla*/
  $("#profesor").on('change', function(){

    $('.datepicker-here').val(calcular_semana($('.datepicker-here').val()));

    $("#zona option").remove();
    cargar_zonas( $('.datepicker-here').val() );
  ...
  /*funcion que borra todas las entradas del resumen*/
  ...
  {
    profe = $("#profesor").val();
    $.post("/practico/cargar_zonas", {profe:profe}, function(data){
        $("#zona").append(data);
        //alert(data);
  ...
  $("#guardar").on('click', function()
  {
    $(".result").each(function(){
      ...
      $.post("/practico/guardar_practicas", {fecha:fecha,hora:hora,profe:profe,zona:zona}, function(data){
  ...
  /*Si eres profesor te redirecciona a horarios*/
  if( $("#profe").text() == 1 )
  {
    window.location.href = "/practico/horarios";
  }
